...
import { MONSTER_INFO as monsterInfoData } from '@/composables/useMonsterData'
...
// แต่ละธาตุมีหลายจาน สุ่มใหม่ทุกครั้งที่เข้า Canteen (อยู่ในร้านชื่อไม่เปลี่ยน)
const CHEF_DISHES = {
  1: { color: '#ff6a2a', menu: [
    { dish: 'เนื้อย่างลาวา', desc: 'เนื้อชิ้นโตย่างบนหินภูเขาไฟ ร้อนจนไฟลุกท่วมจาน' },
    { dish: 'สเต๊กเปลวเพลิง', desc: 'ย่างไฟแรงจนขอบกรอบ ข้างในยังชุ่มฉ่ำ' },
    { dish: 'ไส้กรอกพริกไฟ', desc: 'เผ็ดจนควันออกหู กินแล้วใจฮึกเหิม' },
    { dish: 'ขาแกะรมควันถ่านแดง', desc: 'รมควันข้ามคืนบนถ่านที่ไม่เคยดับ' },
    { dish: 'ข้าวผัดกระทะร้อน', desc: 'ผัดบนกระทะที่ร้อนจนแดง หอมกลิ่นกระทะไหม้' },
  ] },
  2: { color: '#3aa8ff', menu: [
    { dish: 'ปลาย่างธารน้ำใส', desc: 'ปลาแม่น้ำตัวอ้วนย่างเกลือ ห่อใบไม้ให้ชุ่มฉ่ำ' },
    { dish: 'ซุปหอยทะเลสาบ', desc: 'น้ำซุปใสแจ๋ว หวานจากหอยล้วน ๆ' },
    { dish: 'ปลาดิบสายน้ำเย็น', desc: 'สดจนยังรู้สึกถึงกระแสน้ำ' },
    { dish: 'กุ้งแม่น้ำนึ่งใบบัว', desc: 'นึ่งไอน้ำจากธารน้ำตก หอมกลิ่นใบบัว' },
    { dish: 'ข้าวต้มปลาหิมะละลาย', desc: 'อุ่นท้อง ชุ่มคอ เหมือนดื่มน้ำจากลำธาร' },
  ] },
  3: { color: '#ffd23a', menu: [
    { dish: 'ไก่ทอดสายฟ้า', desc: 'กรอบซ่าทุกคำ กินแล้วขนลุกเหมือนโดนฟ้าผ่า' },
    { dish: 'หมูสามชั้นช็อตไฟ', desc: 'หนังกรอบระเบิดเสียงเปรี๊ยะในปาก' },
    { dish: 'ไข่เจียวฟ้าคะนอง', desc: 'ฟูสูงจนเชฟต้องตะโกน "ถอยไปเมี๊ยว!"' },
    { dish: 'เนื้อเสียบไม้ประกายไฟ', desc: 'ย่างด้วยประกายไฟฟ้า สุกทั่วถึงในพริบตา' },
    { dish: 'ซุปผักซ่าลิ้น', desc: 'ซ่าตั้งแต่คำแรกจนผมตั้ง' },
  ] },
  4: { color: '#9be8ff', menu: [
    { dish: 'ซุปเห็ดหิมะ', desc: 'ซุปเย็นเจี๊ยบจากเห็ดยอดเขา ชื่นใจจนตัวสั่น' },
    { dish: 'เนื้อแช่แข็งเชือดบาง', desc: 'เฉือนบางจนเห็นแสงลอด ละลายในปาก' },
    { dish: 'สลัดผักยอดเขาน้ำแข็ง', desc: 'กรอบเย็นสดชื่น เก็บจากเนินที่หิมะไม่เคยละลาย' },
    { dish: 'ปลาหมักเกลือหิมะ', desc: 'หมักในถ้ำน้ำแข็งหนึ่งฤดู รสเข้มจนหนาวสั่น' },
    { dish: 'พุดดิ้งลมหนาว', desc: 'ของหวานเย็นเฉียบ เชฟบอกว่ากินแล้วไม่กลัวอะไร' },
  ] },
  5: { color: '#c46bff', menu: [
    { dish: 'สตูว์เขี้ยวมังกร', desc: 'ตำรับลับที่เชฟหวงที่สุด เคี่ยวข้ามคืนจนเข้มข้น' },
    { dish: 'เนื้อย่างเกล็ดดำ', desc: 'หนังเกรียมสีดำเงา กลิ่นหอมลึกลับ' },
    { dish: 'ซุปกระดูกโบราณ', desc: 'เคี่ยวจากกระดูกที่ขุดเจอในซากมังกรเก่า' },
    { dish: 'ข้าวหน้าเนื้อราชันย์', desc: 'จานที่เชฟทำให้แค่นักล่าที่ไว้ใจเท่านั้น' },
    { dish: 'หม้อไฟเงามังกร', desc: 'น้ำซุปสีม่วงเข้ม ชิมแล้วรู้สึกถึงพลังโบราณ' },
  ] },
}
const _pick = (list) => list[Math.floor(Math.random() * list.length)]
const _rollChefMenu = () => Object.fromEntries(Object.entries(CHEF_DISHES).map(([id, e]) => [id, { ..._pick(e.menu), color: e.color }]))
const chefMenu = ref(_rollChefMenu())
const chefDish = (el) => chefMenu.value[el?.elemental_id] ?? { dish: el?.elemental ?? '', desc: '', color: '#ffb050' }
...
watch(() => activeLocation.value === 'chef', (inChef) => {
  if (!inChef) return
  chefScene.value = _pick(CANTEEN_SCENES)
  chefMenu.value = _rollChefMenu()
}, { immediate: true })
